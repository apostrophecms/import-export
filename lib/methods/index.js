...
function verifyFormats(formats) {
  ...
  Object
    ...
    .forEach(([ formatName, format ]) => {
      const allowedKeys = [ 'label', 'extension', 'allowedExtension', 'allowedTypes', 'includeAttachments', 'input', 'output' ];
      ...
      keys.forEach(key => {
        if (!allowedKeys.includes(key)) {
          throw new Error(`format.${key} is not a valid key`);
        }
      });
      keys.forEach(key => {
        if (key === 'label') {
          if (typeof format.label !== 'string') {
            throw new Error(`${formatName}.label must be a string`);
          }
        } else if (key === 'extension') {
          if (typeof format.extension !== 'string') {
            throw new Error(`${formatName}.extension must be a string`);
          }
        } else if (key === 'allowedExtension') {
          if (typeof format.allowedExtension !== 'string') {
            throw new Error(`${formatName}.allowedExtension must be a string`);
          }
        } else if (key === 'allowedTypes') {
          if (!Array.isArray(format.allowedTypes)) {
            throw new Error(`${formatName}.allowedTypes must be an array`);
          }
          format.allowedTypes.forEach(allowedType => {
            if (typeof allowedType !== 'string') {
              throw new Error(`${formatName}.allowedTypes must be an array of strings`);
            }
          });
        } else if (key === 'includeAttachments') {
          if (typeof format.includeAttachments !== 'boolean') {
            throw new Error(`${formatName}.includeAttachments must be a boolean`);
          }
        } else if (key === 'input') {
          if (typeof format.input !== 'function') {
            throw new Error(`${formatName}.input must be a function`);
          }
        } else if (key === 'output') {
          if (typeof format.output !== 'function') {
            throw new Error(`${formatName}.output must be a function`);
          }
        }
      });
    });
