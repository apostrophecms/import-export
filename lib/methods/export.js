...
module.exports = self => {
  return {
    async export(req, manager, reporting = null) {
      ...
      const docs = await self.getDocs(req, format, ids, hasRelatedTypes, manager, reporting);
      ...
      if (!format.includeAttachments) {
        return self.writeExportFile(
          req,
          reporting,
          format.formatData(allCleanDocs),
          {
            expiration,
            format
          }
      ...
      return self.writeExportFile(
        req,
        reporting,
        format.formatData(allCleanDocs, cleanAttachments, attachmentUrls),
        {
          expiration,
          format
        }
    ...
    async getDocs(req, format, docsIds, includeRelationships, manager, reporting) {
      if (!docsIds.length) {
        return [];
      }
      ...
      if (self.shouldExportDraft(format)) {
      ...
      if (self.shouldExportPublished(format)) {
    ...
    shouldExportDraft(format) {
      return !format.exportVersions || !format.exportVersions.length || format.exportVersions.includes('draft');
    },

    shouldExportPublished(format) {
      return !format.exportVersions || !format.exportVersions.length || format.exportVersions.includes('published');
    },
