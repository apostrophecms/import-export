...
const { detectDocChange } = require('apostrophe/modules/@apostrophecms/schema/ui/apos/lib/detectChange.js');
  ...
  return {
    ...
    async insertOrUpdateDocWithKey(req, {
      ...
      async function update() {
        ...
        if (matchingDraft) {
          const modified = importDraftsOnly && detectDocChange(manager.schema, matchingDraft, matchingPublished);
    ...
    async insertOrUpdateDoc(req, {
      ...
      async function update() {
        ...
        if (doc.aposMode === 'draft') {
          // By default, we set the draft as non-modified.
          let modified = false;

          // However, if the import is set to import drafts only,
          // and ONLY if there is a change between the draft and the published version,
          // we set the draft as modified.
          if (importDraftsOnly) {
            const publishedDoc = await self.apos.doc.db.findOne({
              aposDocId: doc.aposDocId,
              aposLocale: doc.aposLocale,
              aposMode: 'published'
            });
            console.log('🚀 ~ update ~ publishedDoc:', publishedDoc);

            modified = detectDocChange(manager.schema, doc, publishedDoc);
            console.log('detectDocChange(manager.schema, doc, publishedDoc)', detectDocChange(manager.schema, doc, publishedDoc));
            console.log('🚀 ~ update ~ modified:', modified);
          };

          await self.apos.doc.db.updateOne({ _id: docToUpdate._id }, {
            $set: {
              modified: Boolean(modified)
            }
          });
