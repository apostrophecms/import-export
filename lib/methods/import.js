...
module.exports = self => {
  return {
    ...
    async checkDuplicates(req, {
      ...
      for (const {
        type, aposDocId, parkedId, title, updatedAt, aposLocale
      } of docs) {
        ...
        if (isSingleton || parkedId) {
          if (!aposLocale.endsWith(':draft') || !aposDocId) {
            continue;
          }
    ...
    async insertOrUpdateDoc(req, {
      ...
      self.replaceDocId(doc, existingAposDocId);
      self.handleRichTextFields(manager, doc);
    ...
    async overrideDuplicates(req) {
      ...
      for (const doc of filterDocs) {
        try {
          ...
          const replaceItem = replaceAposDocIds.find(([ importId ]) => importId === doc.aposDocId);
          await self.insertOrUpdateDoc(req, {
            doc,
            method: 'update',
            failedIds,
            existingAposDocId: replaceItem?.[1]
          });

          if (replaceItem) {
            replaceDocIdPairs.push([
              doc._id,
              `${replaceItem[1]}:${doc.aposLocale}`
            ]);
          }
    ...
    replaceDocId(doc, replaceId) {
      if (!replaceId) {
        return doc;
      }
      const oldAposDocId = doc.aposDocId;
      doc._id = `${replaceId}:${doc.aposLocale}`;
      doc.aposDocId = replaceId;

      const manager = self.apos.doc.getManager(doc.type);
      if (manager && self.isPage(manager)) {
        doc.path = doc.path.replace(oldAposDocId, replaceId);
        if (doc.aposLastTargetId) {
          doc.aposLastTargetId = doc.aposLastTargetId.replace(oldAposDocId, replaceId);
        }
      }
