...
module.exports = self => {
  const storage = {
    download: async (id) => {
      await util.promisify(self.apos.attachment.uploadfs.copyOut)('/import-export', path.basename(file));
    },
    upload: async (id, file) => {
      await util.promisify(self.apos.attachment.uploadfs.copyIn)(path.basename(file), '/import-export');
    },
    remove: async (id) => {
      await self.remove(exportPath);
      await util.promisify(self.apos.attachment.uploadfs.remove)('/import-export', path.basename(file));
    }
  };
  ...
  return {
    async import(req, moduleName) {
      ...
      try {
        if (overrideLocale) {
          ...
          await storage.download(await self.getExportId(exportPath));
          ...
          await storage.upload(exportId, file);
          exportPath = await self.getExportPath(exportId);

          ({ docs, attachmentsInfo = [] } = await format.input(file.path));
        }
    ...
    async setExportId(exportPath) {
      const id = self.apos.util.generateId();
      const filepath = path.join(self.apos.attachment.uploadfs.getTempPath(), exportPath);

      await self.apos.cache.set('exportPaths', id, filepath, 86400);
      await self.apos.cache.set('exportPathIds', filepath, id, 86400);
      return id;
    },
    ...
    async getExportId(exportPath) {
      return self.apos.cache.get('exportPathIds', exportPath);
    },
