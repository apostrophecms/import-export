...
module.exports = self => {
  // NOTE: will store imported raw file in uploadfs
  const storage = {
    download: async (id) => {
      const exportPath = await self.getExportPath(id);
      const target = path.resolve('/import-export', path.basename(id), exportPath);
      const local = path.resolve('/tmp', target);
      if (await fs.access(local, fs.constants.R_OK | fs.constants.W_OK)) {
        return;
      }

      await util.promisify(self.apos.attachment.uploadfs.copyOut)(target, local);
    },
    ...
    remove: async (id) => {
      const target = path.resolve('/import-export', path.basename(id));
      const local = path.resolve('/tmp', target);
      await self.remove(local);
