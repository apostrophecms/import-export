  ...
  return {
    async import(req, moduleName) {
      ...
      if (exportPath) {
        ...
        const exportFile = await self.readExportFile(req, format, file);
        console.log('🚀 ~ import ~ exportFile:', exportFile);
      ...
      const total = format.includeAttachments && attachmentsInfo?.length
      ...
      if (format.includeAttachments && attachmentsInfo?.length) {
    ...
    async readExportFile(req, format, file) {
      try {
        const exportPath = await format.input(file.path);
        console.log('🚀 ~ readExportFile ~ exportPath:', exportPath);
    ...
    // TODO: duplicatedDocs are not detected
    ...
    async overrideDuplicates(req) {
      ...
      const failedIds = [];
      // console.log('🚀 ~ overrideDuplicates ~ formatLabel:', formatLabel);
