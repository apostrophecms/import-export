...
module.exports = self => {
  return {
    async import(req, moduleName) {
      ...
      try {
        if (exportPath) {
          ...
          // If no export path is found, it means the user is importing a file for the first time.
          // A format might need to extract the file content into a temporary directory, so we need to
          // store the path to be able to clean it up later.
          // By default, the export path is the same as the file path.
          // In the case of an archive, the export path won't be the same as the file path because
          // it will be extracted into a temporary directory. That directory will be the export path.
          ({
            docs = [],
            attachmentsInfo = [],
    ...
    shouldImportDraft(format) {
      return !format.importVersions?.length || format.importVersions.includes('draft');
    },

    shouldImportPublished(format) {
      return !format.importVersions?.length || format.importVersions.includes('published');
    },
    ...
    async overrideDuplicates(req) {
      ...
      const filterDocs = docs.filter(({ aposDocId }) => docIds.includes(aposDocId));
