...
module.exports = self => {
  const storage = {
    download: async (id) => {
      const artifactsPath = await self.setExportArtifactsPath(id);
      if (await fs.access(artifactsPath, fs.constants.R_OK)) {
        return;
      }
  ...
  return {
    ...
    async getExportArtifactsPath(id) {
      await self.apos.cache.get('exportArtifactsPath', id);
    },
