...
module.exports = self => {
  return {
    ...
    async endJob(jobId, sucess, results) {
      const jobManager = self.apos.modules['@apostrophecms/job'];
      await jobManager.end({ _id: jobId }, sucess, results);
    },
      ...
      for (const {
        ...
        if (!self.canImport(req, type)) {
          ...
          failedLog[aposDocId] = {
            ...
            title,
            aposLocale,
            detail: req.t('aposImportExport:errorCantImportExportType', { type })
          };
          reporting.failure();
          continue;
        }
    ...
    async insertDocs(req, {
      docs, reporting, duplicatedIds, duplicatedDocs, failedIds, failedLog, importDraftsOnly
    }) {
      for (const doc of docs) {
