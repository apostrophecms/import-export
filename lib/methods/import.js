...
module.exports = self => {
  return {
    async import(req, moduleName) {
      ...
      const { duplicatedDocs, duplicatedIds } = await self.checkDuplicates(req, docs);
      ...
      const failedIds = await self.insertDocs(req, {
        docs,
        reporting,
        duplicatedIds
      });
    ...
    async checkDuplicates(req, docs) {
      // TODO: Permission?

      const duplicatedDocs = [];

      // TODO: Use toAposDocId
      const docIds = docs
        .filter(({ aposDocId }) => aposDocId)
        .map(({ aposDocId }) => aposDocId);

      const duplicates = await self.apos.doc.db
        .distinct('aposDocId', { aposDocId: { $in: docIds } });

      const duplicatedIds = new Set(duplicates);

      for (const doc of docs) {
        if (duplicatedIds.has(doc.aposDocId) && doc.aposMode === 'draft') {
          duplicatedDocs.push({
    ...
    async insertDocs(req, {
      docs, reporting, duplicatedIds
    }) {
      const failedIds = [];

      for (const doc of docs) {
        if (duplicatedIds.has(doc.aposDocId)) {
          continue;
        }
    ...
    async insertOrUpdateAttachment(req, {
      ...
      if (duplicatedIds.size && docIds) {
        ...
        if (relatedIds.every((id) => duplicatedIds.has(id))) {
      ...
      try {
        ...
        // TODO: Should be treated as duplicated like normal docs
