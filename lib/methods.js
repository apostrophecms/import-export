module.exports = self => {
  return {
    async export(req, manager) {
      ...
      const allAttachmentsIds = allDocs.flatMap(doc => {
        return self.getRelatedIdsBySchema({
          doc,
          schema: self.apos.modules[doc.type].schema,
          type: 'attachment'
    ...
    getRelatedIdsBySchema({
      ...
      const related = schema.reduce((acc, field) => {
        ...
        if (field.type === 'array') {
          concatSet(acc, fieldValue.flatMap((subField) => self.getRelatedIdsBySchema({
            doc: subField,
            schema: field.schema,
            type
          }))
        ...
        if (field.type === 'object') {
          concatSet(acc, self.getRelatedIdsBySchema({
            doc: fieldValue,
            schema: field.schema,
            type
          }));
        ...
        if (field.type === 'area') {
          concatSet(
            acc,
            (fieldValue.items || []).flatMap((widget) => {
              return self.getRelatedIdsBySchema({
                doc: widget,
        ...
        if (field.type === type) {
          concatSet(
            acc,
            Array.isArray(fieldValue) ? fieldValue.map(({ _id }) => _id) : [ fieldValue._id ]
          );
        }
