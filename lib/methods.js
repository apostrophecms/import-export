...
module.exports = self => {
  return {
    ...
    generateZipFile(data) {
      return new Promise((resolve, reject) => {
        try {
          const dirPath = path.join(process.cwd(), 'data/temp');
          const zipPath = path.join(dirPath, `aposExport-${self.apos.util.generateId()}.zip`);
          ...
          const docsBuffer = Buffer.from(data.docs, 'utf8');
          archive.append(docsBuffer, { name: 'aposDocs.json' });

          if (data.attachments) {
            const attachmentBuffer = Buffer.from(data.attachments, 'utf8');
            archive.append(attachmentBuffer, { name: 'aposAttachments.json' });
          }
